    ...
    class DefaultSessionDefinitions : ISessionDefinitionsSource
    {
        ...
        private static SessionDefinition CreateDefaultDefinition(IEnumerable<DeviceType> deviceTypes)
        {
            ...
            if (hasEyeTracker)
            {
                ...
                definition.SessionSteps.Add(new SessionStep() { Action = new EyeTrackerValidationActionSettings() { PointDuration = TimeSpan.FromSeconds(1) }, Completion = new SessionStepCompletionSettings() { Hotkeys = new List<Hotkey>() { Hotkey.F10 } } });
            }
